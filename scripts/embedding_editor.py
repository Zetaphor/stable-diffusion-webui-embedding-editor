...
def on_ui_tabs():
    ...
    with gr.Blocks(analytics_enabled=False) as embedding_editor_interface:
        ...
        btn_find_similar_distribution.click(
            fn=find_similar_distribution,
...
def load_distribution_weights():
    ...
    with open(os.path.join(os.getcwd(), 'extensions/stable-diffusion-webui-embedding-editor/weights', sd_version + '-weight-distribution.pkl'), 'rb') as f:
        distribution_weights = pickle.load(f)
...
def find_similar_distribution(*weights):
    distribution_weights = load_distribution_weights()
    print(distribution_weights[1][0])
    return []
